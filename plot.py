...
cv_df = pd.read_csv('output/cv_results.out', names=['d', 'k', 'error']).apply(pd.to_numeric, errors='coerce')
...
cv_best_df = pd.read_csv('output/cv_best.out', names=['d', 'k', 'error', 'nsv', 'nbsv']).apply(pd.to_numeric, errors='coerce')
test_best_df = pd.read_csv('output/test_best.out', names=['d', 'k', 'error']).apply(pd.to_numeric, errors='coerce')
...
plt.xlabel('Degree d')
plt.ylabel('Error (%)')
plt.title('Errors vs. Polynomial Kernel Degree for C=$\mathregular{2^7}$')
plt.savefig("plots/cv_vs_test.png")
plt.close()



cv_best_df = cv_best_df.groupby(['d','k']).mean().reset_index()


plt.plot(cv_best_df['d'], cv_best_df['nbsv'], label='Bounded SVs')
plt.plot(cv_best_df['d'], cv_best_df['nsv'], label='All SVs')

plt.legend()
plt.grid(True)
plt.xlabel('Degree d')
plt.ylabel('Frequency')
plt.title('Number of Support Vectors (SV) vs. Polynomial Kernel Degree=d')
plt.savefig("plots/support_vectors.png")
plt.close()
